//
// Created by Devin AI on 6/11/25.
//

#pragma once
#ifndef VKHASHDAG_VOXIMPORTER_HPP
#define VKHASHDAG_VOXIMPORTER_HPP

#include "VBREditor.hpp"
...
struct VoxImporter {
	...
	inline EditType EditNode(const Config<uint32_t> &config,
	                         const NodeCoord<uint32_t> &coord, NodePointer<uint32_t> ptr,
	                         VBRColor &color) const {
		...
		for (const auto& voxel : voxels) {
			...
			if (glm::all(glm::greaterThanEqual(vpos, lb)) && glm::all(glm::lessThan(vpos, ub))) {
				return EditType::kProceed;
			}
		}
		return EditType::kNotAffected;
	}
	
	inline bool EditVoxel(const Config<uint32_t> &config, const NodeCoord<uint32_t> &coord,
	                      bool voxel, VBRColor &color) const {
...
#endif // VKHASHDAG_VOXIMPORTER_HPP
